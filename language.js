...
const ru = {
    ...
    // Карточки маршрутов
    trail1_name: "Воробьёвы горы",
    trail1_desc: "Легендарный маршрут по Москве-реке с панорамным видом на город. Проходит через парк, набережную и смотровые площадки.",
    trail2_name: "Куршская коса",
    trail2_desc: "Уникальный маршрут по национальному парку между морем и заливом. Песчаные дюны, сосновый лес и побережье Балтики.",
    trail3_name: "Лаго-Наки",
    trail3_desc: "Высокогорный маршрут по альпийским лугам Адыгеи. Горы, водопады и виды, которые захватывают дух.",
    trail4_name: "Алтайский Марс",
    trail4_desc: "Космические пейзажи Алтая: красные скалы, бирюзовые реки и горные тропы.",
    trail5_name: "Байкальская петля",
    trail5_desc: "Кольцевой маршрут вдоль озера Байкал с заездом на пик Черского.",
    trail6_name: "Долина гейзеров",
    trail6_desc: "Экстремальный маршрут Камчатки. Термальные источники, вулканы, медвежьи тропы.",
...
const en = {
    ...
    // Trail cards
    trail1_name: "Vorobyovy Gory",
    trail1_desc: "Legendary route along the Moscow River with panoramic views of the city. Passes through the park, embankment and observation platforms.",
    trail2_name: "Curonian Spit",
    trail2_desc: "Unique route through the national park between the sea and the bay. Sand dunes, pine forest and Baltic coast.",
    trail3_name: "Lago-Naki",
    trail3_desc: "High-altitude route through the alpine meadows of Adygea. Mountains, waterfalls and breathtaking views.",
    trail4_name: "Altai Mars",
    trail4_desc: "Cosmic landscapes of Altai: red rocks, turquoise rivers and mountain trails.",
    trail5_name: "Baikal Loop",
    trail5_desc: "Circular route along Lake Baikal with a visit to Chersky Peak.",
    trail6_name: "Valley of Geysers",
    trail6_desc: "Extreme route in Kamchatka. Thermal springs, volcanoes, bear trails.",
...
function updateAllTexts() {
    ...
    if (modalTitle) {
        ...
        modalTitle.innerHTML = t(isLogin ? 'modal_login' : 'modal_register');
        
        const submitBtn = document.getElementById('submitBtn');
        if (submitBtn) submitBtn.innerHTML = t(isLogin ? 'modal_submit_login' : 'modal_submit_register');
        
        const switchMode = document.getElementById('switchMode');
        if (switchMode) switchMode.innerHTML = t(isLogin ? 'modal_switch_login' : 'modal_switch_register');
...
function updateMenuTexts() {
    ...
    menuItems.forEach(item => {
        ...
        if (text.includes('Личный кабинет') || text.includes('Profile')) {
            ...
            if (currentLang === 'ru') {
                item.innerHTML = isLight ? '🌙 Тёмная тема' : '☀️ Сменить тему';
            } else {
                item.innerHTML = isLight ? '🌙 Dark theme' : '☀️ Change theme';
            }
...
// Смена языка с перезагрузкой страницы
